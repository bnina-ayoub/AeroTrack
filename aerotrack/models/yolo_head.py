...
from aerotrack.utils import bboxes_iou, cxcywh2xyxy, meshgrid, visualize_assign
...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.iou_loss = IOUloss(reduction="none", loss_type="iou", alpha=3.0)
    ...
    def get_assignments(
        ...
        if fg_mask.sum() == 0:
            return (
                torch.zeros(0, dtype=torch.int64, device=fg_mask.device),
                fg_mask,
                torch.zeros(0, dtype=torch.float, device=fg_mask.device),
                torch.zeros(0, dtype=torch.int64, device=fg_mask.device),
                0,
            )
